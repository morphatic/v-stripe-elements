...
import Vue from 'vue'; // import { VuetifyObject } from 'vuetify/types' // !this causes type errors if imported
// Styles
...
export default base.extend().extend({
  ...
  watch: {
    isDark(val, oldVal) {
      ...
      if (val !== oldVal && this.card !== null) {
        ...
        const style = this.genStyle(this.font, this.$vuetify.theme.currentTheme, this.$vuetify.theme.dark); // then update the card
  ...
  mounted() {
    ...
    .then(() => {
      this.elements = this.stripe && this.stripe.elements(this.genFont(this.font));
  ...
  methods: {
    ...
    async createToken() {
      ...
      const {
        token,
        error
      } = await this.stripe.createToken(this.card, this.tokenOptions);
    ...
    async verifyCardInfo() {// // bail if we're not ready yet
