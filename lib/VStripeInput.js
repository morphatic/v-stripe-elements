...
import { VProgressLinear, VTextField } from 'vuetify/lib'; // Mixins
...
export default Vue.extend({
  ...
  mounted() {
    ...
    .then(() => this.stripe.elements(this.genFont(this.font))) // then create a card element
    .then(elements => elements.create('card', cardProps)) // then setup card events and mount the card
    .then(card => {
      this.card = card;
  ...
  methods: {
    ...
    async createToken() {
      const {
        token,
        error
      } = await this.stripe.createToken(this.card, this.nameAndAddress);
    ...
    async verifyCardInfo() {
      ...
      if (!this.okToSubmit) return; // otherwise, submit info to Stripe

      const {
        source,
        error
      } = await this.stripe.createSource(this.card, {
        currency: 'usd',
        metadata: this.meta,
        owner: this.owner,
        usage: 'reusable'
